    ...
    <section id="contact" className="section">
      ...
      <div className="contact-cards">
        ...
        <motion.a
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.20, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
        >
        ...
        {/* PGP Public Key */}
        <motion.a
          href="https://keys.openpgp.org/vks/v1/by-fingerprint/5238B3BF98426B8BD805FA15FA85D9582C7CB3B8"
          target="_blank"
          rel="noopener noreferrer"
          className="glass-card contact-link contact-link--pgp"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ delay: 0.40, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
        >
          <Key size={20} color="var(--accent-amber)" />
          <span style={{ flex: 1 }}>{t.contact.pgpKey}</span>
          <ArrowRight size={16} style={{ opacity: 0.4 }} />
        </motion.a>
      </div>
    </section>
